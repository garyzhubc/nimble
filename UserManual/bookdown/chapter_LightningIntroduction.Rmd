...
We'll use the *pump* model example from BUGS\footnote{The data set
...
`lifted\_d1\_over\_beta`. We call this a ``lifted'' node. Like R,
...
NIMBLE is a system for working with algorithms, not just an MCMC engine. So let's try maximizing the marginal likelihood for `alpha` and `beta` using Monte Carlo Expectation Maximization\footnote{Note that for this model, one could analytically integrate over `theta` and then numerically maximize the resulting marginal likelihood.}. 
