...
Some of the useful tools and objects to create in setup functions include:
  ...
  *[vectors of node names, often from a model] Often these are obtained from the
  `getNodeNames`, n`getDependencies`, and other methods of a model,
...
empty setup code, use `setup = function() \{\`} or \cd{setup =
  TRUE}.  
...
```
  ...
  %% 
  %%   *[`from`, `to`] which can either be models or modelValues
  %%   *[`row`, `rowTo`] which refers to the rows of a
  ...
  %%     *[`nodes`, `nodesTo`] which is a vector of the names of the nodes
