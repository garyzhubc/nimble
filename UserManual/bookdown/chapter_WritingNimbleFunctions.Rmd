...
are two kinds of code and two steps of execution:
  ...
  * *Setup* code is provided as a regular R function, but the
  ...
  * *Run* code is provided in the NIMBLE language, which was
  ...
  kinds of *run* code:
    ...
    * There is always a primary function, given as the argument `run`
    ^[This can be omitted if you don't need it.].
    * There can optionally be other functions, or 'methods' in the
...
Some of the useful tools and objects to create in setup functions include:
  ...
  *[vectors of node names, often from a model] Often these are obtained from the
  `getNodeNames`, 
`getDependencies`, and other methods of a model,
...
<!---  Comment: It is actually sketchy that we allow a nimbleList -->  % definition to be created in setup code, because it would  have to
<!---  be the same definition always used by all specializations.  So -->  % I've put the emphasis here on creating nimbleList objects. -PdV
...
empty setup code, use `setup = function() \{\`} or `setup =
  TRUE`.  
...
### Driving models: `calculate, \cd{calculateDiff`,
  `simulate`, `getLogProb`} {#sec:driv-models:-calc}
...
subsets of nodes, or variables.  Note the following:
   ...
   * one should use the ````{r, -` global assignment operator to assign into model nodes. 
...
\subsubsection{Getting and setting more than one model node or variable
  at a time using `values`} {#sec:getting-setting-more}
...
<!--- %   *[`from`, `to`] which can either be models or modelValues -->  %%   *[`row`, `rowTo`] which refers to the rows of a
<!--- %   modelValues object, if either `from` or `to` is a -->  %%   modelValues. If `rowTo` is omitted, it is assumed to be equal to `row` if necessary.
<!--- %     *[`nodes`, `nodesTo`] which is a vector of the names of the nodes -->  %%     to be copied.  The node names will be expanded when variable names are provided.  If
...
scalar nodes.  Instead, a single node such as defined by `z $\sim$
...
as a simple class inheritance system.  In Version `r version`\ it is limited to
...
<!--- % ```{r, copy-model} -->%% newPump <- pumpModel$newModel()
<!--- % ```  -->
...
If your nimbleFunctions are very fast, say under 1ms, then `microbenchmark` will be inacurate due to R-to-C++ conversion overhead (that won't happen in your actual functions).
...
use -- in the future.
<!--- % ### rankSample -->%% \label{sec:ranksample}
