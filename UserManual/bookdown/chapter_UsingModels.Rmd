...
We can determine the set of nodes contained in one or more nodes or variables using `expandNodeNames`, illustrated here for an example with multivariate nodes. Then`returnScalarComponents` argument also allows us to return all of the scalar elements of multivariate nodes. 
...
generated.  These include:
  ...
  * When distribution parameters are expressions, NIMBLE
  ...
  * As discussed in Section @ref(subsec:BUGS-link), the use of link
  ...
  * Use of alternative parameterizations of distributions, described in Section @ref(subsec:dists-and-functions) causes new nodes to be introduced. For
...
## Carrying out model calculations
  %`calculate`, `calculateDiff`, `simulate`, and `getLogProb` {#sec:cdcalc-cdsim-cdgetl}
...
`simulate` is equivalent to `calculate` without returning
0. `simulate` always returns `NULL` (or `void` in C++).
...
vector.
  * Simulations will be done in the order provided, so in practice
  ...
  it depends on.
  * The data nodes `z` were not simulated into until
...
### Pre-defined nimbleFunctions for operating on model nodes: `simNodes, \cd{calcNodes`, and `getLogProbNodes`} {#sec:cdsimn-cdcalcn-cdget}
