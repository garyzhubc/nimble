...
```
!<---```{r, child = 'eigenSvdTableMatrixAsNimList.md'}
``` -->
...
now we've included just one additional useful feature:  `for(i in
...
`inprod(v1, v2)`. 
  ...
  * The left-hand side of an assignment can use indexing, but if so
  ...
  already at least 10x30 in size.  This can be done by `setSize(Y,
    c(10, 30))`.  See Section \@ref(sec:chang-sizes-exist) for more
  details.  Note that non-indexed assignment to `Y`, such as `Y
...
halt execution.  Therefore, if you execute `A <- M1 \% * \%
...
code that will crash is to do things like `Y[5:10, 20:30] <-
