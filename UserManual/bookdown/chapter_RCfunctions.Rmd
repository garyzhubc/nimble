...
To write nimbleFunctions, you will need to learn:
  ...
  * that compiled nimbleFunctions always pass arguments to each
  other by reference.
%  * how to use nimbleList data structures.
...
\LTcapwidth=\textwidth
[c]{ll}
 \caption{Basic R manipulation functions in NIMBLE.  To find help in R for NIMBLE's version of a function, use the 'nim' prefix and capitalize the next letter.  E.g. `help(nimC)` for help with `c()`.} \label{table:functions-coreR} n
 \hline
...
#### `numeric, \cd{integer`, `logical`, `matrix`
  and `array`}
...
#### `length and \cd{dim`} {#sec:query-chang-sizes}
...
[c]{lllcc}
 \caption{Functions operating on scalars, many of which can operate on
   ...
   NIMBLE.}    \label{table:functions-dsl}n
\hline
 Usage & Description & Comments & Status & Accepts n
   &  &  &  & vector input  n
  \hline \hline n
\endhead
...
[c]{lllc}
 \caption{Functions operating on vectors and matrices. Status column
  ...
  NIMBLE.} \label{table:functions-matrix-dsl} n
  \hline
  Usage & Description & Comments & Status   n
  \hline \hline n
\endhead
...
from nimbleFunctions (and in BUGS model code), but care is needed in the syntax, as follows.
  ...
  * Supported parameterizations are also indicated in the list below. 
    * For multivariate distributions (multivariate normal,
...
{2}
  ...
  * `dnorm(x, mean, sd, log)`
   ...
   * `dweibull(x, shape, scale, log)`
    ...
    * `dwish_chol(x, cholesky, df, nscale_param, log)`
...
### Flow control: `if-then-else, \cd{for`,  `while`, and `stop`} {#sec:basic-flow-control}
...
now we've included just one additional useful feature:  \cd{for(i in
  seq_along(NFL))} will work as in R, where `NFL` is a
...
### Optimization: `optim and \cd{nimOptim`}
...
#### Confusions between vectors and one-column or one-row matrices {#sec:conf-betw-scal}
...
`inprod(v1, v2)`. 
  ...
  * The left-hand side of an assignment can use indexing, but if so
  ...
  details.  Note that non-indexed assignment to `Y`, such as \cd{Y
    <- x}, will automatically set `Y` to the necessary size. 
...
to write code that will crash your R session.  In Version `r version`, NIMBLE
...
prevent crashes, but in Version `r version`\ we limit ourselves to trying to provide
useful information.
...
`sizes` is an optional vector of fixed, known sizes.  
 For example, \cd{double(2,
  c(4, 5))} declares a `4-$\times$-5` matrix.  If sizes
